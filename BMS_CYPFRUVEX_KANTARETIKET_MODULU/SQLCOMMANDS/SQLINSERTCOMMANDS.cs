...
SqlCommand com = new SqlCommand("INSERT INTO "+tableName + "(" + (INSERTLOGICALREF ? "LOGICALREF, " : "")+"TARIH, PLAKAID, KONTRAKTORID, URETICIID, URUNID, MIKTAR, BIRIM, KULLANICI, BARKODYAZIMMIKTAR, ACIKLAMA, LOGOAKTARIMI, LOGOAKTARIMTARIHI, LOGOFISID, ERRORMESSAGE, TSTATUS) VALUES ("+(INSERTLOGICALREF ? "@LOGICALREF, " : "") +"@TARIH, @PLAKAID, @KONTRAKTORID, @URETICIID, @URUNID, @MIKTAR, @BIRIM, @KULLANICI, @BARKODYAZIMMIKTAR, @ACIKLAMA, @LOGOAKTARIMI, @LOGOAKTARIMTARIHI, @LOGOFISID, @ERRORMESSAGE, @TSTATUS) SELECT " + 
 (INSERTLOGICALREF ? "MAX(LOGICALREF) FROM " + tableName + "" : " SCOPE_IDENTITY() "));
 try { if (INSERTLOGICALREF) 
 com.Parameters.AddWithValue("@LOGICALREF", B.LOGICALREF); } catch {  com.Parameters.AddWithValue("@LOGICALREF",  DBNull.Value); }
...
if (B.CANCELDATE != new DateTime()) 
 ...
 try { com.Parameters.AddWithValue("@ATAXEXCEPTREASON", B.ATAXEXCEPTREASON); } catch {  com.Parameters.AddWithValue("@ATAXEXCEPTREASON",  DBNull.Value); }
 try { com.Parameters.AddWithValue("@TAXFREECHX", B.TAXFREECHX); } catch {  com.Parameters.AddWithValue("@TAXFREECHX",  DBNull.Value); }
 try { com.Parameters.AddWithValue("@MNTORDERFREF", B.MNTORDERFREF); } catch {  com.Parameters.AddWithValue("@MNTORDERFREF",  DBNull.Value); }
 try { com.Parameters.AddWithValue("@PRINTEDDESPFCNO", B.PRINTEDDESPFCNO); } catch {  com.Parameters.AddWithValue("@PRINTEDDESPFCNO",  DBNull.Value); }
...
if (B.FUTMONTHENDDATE != new DateTime()) 
 ...
 try { com.Parameters.AddWithValue("@KKEGACCREF", B.KKEGACCREF); } catch {  com.Parameters.AddWithValue("@KKEGACCREF",  DBNull.Value); }
 try { com.Parameters.AddWithValue("@KKEGCENTREF", B.KKEGCENTREF); } catch {  com.Parameters.AddWithValue("@KKEGCENTREF",  DBNull.Value); }
 try { com.Parameters.AddWithValue("@MNTORDERFREF", B.MNTORDERFREF); } catch {  com.Parameters.AddWithValue("@MNTORDERFREF",  DBNull.Value); }
...
if (B.MATCHDATE != new DateTime()) 
 ...
 try { com.Parameters.AddWithValue("@VATFLAG", B.VATFLAG); } catch {  com.Parameters.AddWithValue("@VATFLAG",  DBNull.Value); }
